...
另外，有一个叫函数表(symbol table)的概念，这是编译器用于内部维护，以及会导出到object file和exectuable的。例如如果写了多个名为`display`的函数，且传参不同，则可以编译一个object file并用`nm`命令查看，可以看到编译器内部对`display`函数的不同命名。这里的`T`指的是`Text section`，代码段。
...
一个值得注意的点是：不会存在两个函数仅返回值类型不同而其他全部相同（这是不允许的）。

重写(override)指的是在子类中重写父类里的相同函数（同名，同参数，同返回值），且该父类函数**必须是虚函数**，而子类的函数重写后**默认也是虚函数**。虚函数只能是非静态的函数，这是因为重写需要利用虚函数表实现，是一个运行时多态的过程，然而静态的函数不会经历这一步骤，故此不能是虚函数。
...
派生类重写虚函数时函数的签名应该与基类的虚函数一致，并在定义处加```override```关键字。例如：
...
隐藏(hiding/shadowing)指的是子类声明与父类同名的函数，则父类中所有该名字的函数**都会被隐藏**。这个现象的原因是局部作用域，名字查找并不按照对象内存布局走，而是根据作用域逐级查找。编译器首先在本层进行名字查找，查找到的结果再进行重载分析，因此看不到父类的情况，导致父类的函数被隐藏。
...
Child c;
c.f(1);  // 报错：no matching function for call to 'Child::f(int)'
```

假如想要解决这个问题，例如子类中仅定义了`char`的处理，而希望保留父类中对`int`和`double`的处理。可以考虑在子类中加入一行`using Parent::f;`。
