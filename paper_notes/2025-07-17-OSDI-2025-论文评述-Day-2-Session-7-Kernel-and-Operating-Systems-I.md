...
Determinism在智能合约，区块链和可复制状态机等场景下非常重要。主旨是同一程序在给定相同输入的情况下，无论何时、何地或在哪个节点执行，都必须产生完全一致的输出与状态变化。现有工作基于WASM和JIT，有性能开销且TCB较大。本论文推出DeCl (deterministic client)，不依赖中间语言，直接在原生机器码上强制determinism。利用了SFI (software fault isolation)技术。
